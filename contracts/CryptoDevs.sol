...
contract CryptoDevs is ERC721Enumerable, Ownable {
    ...
    function mint() public payable {
        ...
        // totalSupply() function comes from ERC721Enumerable, it is initialized with the value of 0 and then keeps incrementing everytime mint function is called
        require(
        ...
        // Keep in mind that whitelisted users dont have to pay the 0.01 ETH price to mint the NFT, so we check if the msg.value is smaller than the _price
        if (whitelist.whitelistedAddresses(msg.sender) && msg.value < _price) {
            ...
            // Non-whitelisted users must pay the 0.01 mint fee!
